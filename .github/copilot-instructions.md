...
- Checked for common assistant config files (CLAUDE.md, AGENTS.md, CONVENTIONS.md, AIDER_CONVENTIONS.md, .windsurfrules, .cursorrules, .clinerules). None detected in repository root.

---

If helpful, Copilot prompts to bootstrap changes

- "Add an endpoint to /api that returns current warp points. Update internal/state to expose a thread-safe getter and wire it in internal/api. Add unit tests under internal/state." (Adjust path and test names to match repo structure.)

---

Last notes

- This file focuses on repository-specific mechanics: build ordering, where to find handlers, and the embed requirement. Keep it up to date if build or port settings change.
