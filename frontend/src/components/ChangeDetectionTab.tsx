    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-end', mb: 3, flexWrap: 'wrap' }}>
        <FormControl size="small" sx={{ minWidth: 200 }}>
          ...
          >
            {(['basic', 'subtraction', 'threshold', 'heatmap'] as Method[]).map((m) => (
      ...
      {data && (
        <>
          ...
          <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
            {method === 'basic' && (
              <>
                {images?.diff_map && <ResultImage src={images.diff_map} caption="Difference Map" />}
                {images?.threshold_mask && <ResultImage src={images.threshold_mask} caption="Thresholded Changes" />}
                {images?.highlight && <ResultImage src={images.highlight} caption="Changes Highlighted on After" />}
              </>
            )}
            {method === 'subtraction' && (
              <>
                {images?.subtraction && <ResultImage src={images.subtraction} caption="Subtraction Result" />}
                {images?.highlight && <ResultImage src={images.highlight} caption="Changes Highlighted on After" />}
              </>
            )}
            {method === 'threshold' && (
              <>
                {images?.threshold_mask && <ResultImage src={images.threshold_mask} caption={`Threshold Result (sensitivity=${sensitivity})`} />}
                {images?.highlight && <ResultImage src={images.highlight} caption="Changes Highlighted on After" />}
              </>
            )}
            {method === 'heatmap' && (
              <>
                {images?.heatmap && <ResultImage src={images.heatmap} caption="Change Intensity Heat Map" />}
                {images?.highlight && <ResultImage src={images.highlight} caption="Changes Highlighted on After" />}
              </>
            )}
          </Box>
